...
def decode_labels(mask, num_images=12, num_classes=2):
...
def mean_image_addition(image, means=(_R_MEAN*2, _G_MEAN*2, _B_MEAN*2)):
...
def mean_image_subtraction(image, means=(_R_MEAN*2, _G_MEAN*2, _B_MEAN*2)):
...
def eval_input_fn(image_filenames, label_filenames=None, batch_size=12):
  ...
  def _parse_function(filename, is_label):
    ...
    image_string_af = tf.read_file(image_filename[0])
    image_string_bf = tf.read_file(image_filename[1])
    image_af = tf.image.decode_image(image_string_af)
    image_bf = tf.image.decode_image(image_string_bf)
    image_af = tf.to_float(tf.image.convert_image_dtype(image_af, dtype=tf.uint8))
    image_bf = tf.to_float(tf.image.convert_image_dtype(image_bf, dtype=tf.uint8))
    image=tf.concat([image_af,image_bf],axis=2)
    image.set_shape([None,None,6])
