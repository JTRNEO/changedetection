...
parser.add_argument('--image_data_dir_af', type=str, default='afJPEGImages',
                    help='The directory containing the afimage data.')
parser.add_argument('--image_data_dir_bf', type=str, default='bfJPEGImages',
                    help='The directory containing the bfimage data.')
...
def dict_to_tf_example(image_af_path,image_bf_path,
                       label_path):
...
def create_tf_record(output_filename,
  ...
  for idx, example in enumerate(examples):
    ...
    image_af_path = os.path.join(image_dir_af, example + '.jpg')
    image_bf_path = os.path.join(image_dir_bf, example + '.jpg')
    label_path = os.path.join(label_dir, example + '.png')

    if not os.path.exists(image_af_path):
      tf.logging.warning('Could not find %s, ignoring example.', image_af_path)
      continue
    ...
    try:
      tf_example = dict_to_tf_example(image_af_path,image_bf_path, label_path)
...
def main(unused_argv):
  ...
  image_dir_af = os.path.join(FLAGS.data_dir, FLAGS.image_data_dir_af)
  image_dir_bf = os.path.join(FLAGS.data_dir, FLAGS.image_data_dir_bf)
  ...
  create_tf_record(train_output_path, image_dir_af,image_dir_bf, label_dir, train_examples)
  create_tf_record(val_output_path, image_dir_af, image_dir_bf,label_dir, val_examples)
