...
parser.add_argument('--model_dir', type=str, default='./model8',
...
def parse_record(raw_record):
  ...
  image_af = tf.image.decode_image(
      tf.reshape(parsed['image_af/encoded'], shape=[]), _DEPTH)
  image_af = tf.to_float(tf.image.convert_image_dtype(image_af, dtype=tf.uint8))
  image_af.set_shape([None, None, 3])

  image_bf = tf.image.decode_image(
      tf.reshape(parsed['image_bf/encoded'], shape=[]), _DEPTH)
  image_bf = tf.to_float(tf.image.convert_image_dtype(image_bf, dtype=tf.uint8))
  image_bf.set_shape([None, None, 3])
